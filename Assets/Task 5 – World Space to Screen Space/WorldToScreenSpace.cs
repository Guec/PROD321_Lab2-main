...
public class WorldToScreenSpace : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (vertices[i].x == SpherePosition.x && vertices[i].z == SpherePosition.z)
            {
                SpherePosition.y = (int)vertices[i].y + (int)Sphere.transform.localScale.x / 2;
            }
        }

        Sphere.transform.position = transform.TransformPoint(SpherePosition);
    ...
    private void OnGUI()
    {
         ...
         ****/
        // sphere position
        screenPos = renderingCamera.WorldToScreenPoint(Sphere.transform.position);

        // flip y for GUI space
        screenPos.y = Screen.height - screenPos.y;

        Debug.Log("screen" + screenPos);
        Debug.Log("pos" + SpherePosition);
        
        // draw texture
        GUI.DrawTexture(new Rect(screenPos.x - 25, screenPos.y - 25, 50, 50), reticleTexture);
